...
- Regression coverage is end-to-end only: a self-election transaction against a local node, red on the old shape, green on the new. No LEZ node crates enter this repo as dev-dependencies for validation-rule tests, node rules are evidenced by source reference. Using LEZ types for wire-format facts (encoding sizes) is fine, the types are the contract there.
